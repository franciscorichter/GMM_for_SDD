...
gradDescentEvolutionModel <- function(N_max, stats_obs, n_stats, n_pars, learn_rate, patience,
                                      iters, initial_params, n_trees_D, n_trees_sgd, max_t, print_info=TRUE) {
  max_attempts <- 50
  iteration_counter <- 0
  patience_counter <- 0
...
max_t <- 100
max_attempts <- 200  # Maximum attempts for tree generation
...
grad_descent_results <- gradDescentEvolutionModel(N_max, stats_obs, n_stats, n_pars, learn_rate, patience, 
                                                  iters, initial_params, n_trees_D, n_trees_sgd, max_t, print_info=FALSE)
...
ggplot(data.frame(iteration=iterations[-c(1)], value=times_taken), aes(x=iteration, y=value)) +
  geom_line() + theme_bw() + 
...
ggplot(data.frame(iteration=iterations[-c(1)], value=grad_descent_results$stats_diff_abs), aes(x=iteration, y=value)) +
  geom_line() + theme_bw() + 
