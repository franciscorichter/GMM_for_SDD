...
calc_ideal_D_new = function(tp, stats_obs, n_stats, n_pars, pars_i, n_trees_D, ltt_points, times, covariates_list, max_attempts){
  ...
  for (i in 1:n_trees_D){
    ...
    repeat {
      ...
      # Check if nrow(tree_mat) is equal to 2
      if (nrow(tree_mat) != 2) {
        break  # Exit the loop if nrow(tree_mat) is not 2
      }
...
grad_descent_constD = function(N_max, D, M, stats_obs, n_stats, n_pars, learn_rate, patience,
                               iters, pars_i, n_trees_D, n_trees_sgd, covs){
  max_attempts=200
  ...
  while (c < iters){ # CHANGE CONDITION
    ...
    D = calc_D_stats(n_stats,n_pars,pars_i,n_trees_D,covs,max_attempts)
    M = calc_M(D)
    MD = M%*%D
    ...
    for (i in 1:n_trees_sgd){
      attempt = 0
      result = create_tree_mat_fixed_N(N_max, pars_new[1], pars_new[2:length(pars_new)], covs,
    ...
    # update pars
    pars_new = pars_i - learn_rate*(MD%*%(as.matrix(diff)))*abs(pars_i)
...
create_tree_mat_fixed_N <- function(N_max, mu_0, betas, covs, attempt, max_attempts) {
  init = init_tree_mat()
  ...
  while (n_count <= N_max) {
    ...
    if (length(tips_at_t) != 0 & length(left) != 0 & length(right) != 0) {
      ...
      if (attempt >= max_attempts) {
        ...
        sub_result <- create_tree_mat_fixed_N(N_max, mu_0, betas, covs, attempt+1, max_attempts)
    ...
    # sample from NHPP
    if(length(covs)==0){
      t_sample = rexp(1, length(tips_at_t)*
                        (mu(mu_0) + lambda(t, covs, length(tips_at_t), betas)))
    }
    else{
      t_sample = rexp.var(1, function(tm) length(tips_at_t)*(lambda(tm, covs, length(tips_at_t), betas)+mu(mu_0)), 
                        now=t, tMax=Inf, shape=NULL, TS=0)
    }
    ...
    lam = lambda(t, covs, length(tips_at_t), betas)
    
    tot_rate = (mu(mu_0) + lam)*length(tips_at_t)
    ...
    # sample event type
    sum_of_rates = mu(mu_0) + lambda(t, covs, length(tips_at_t), betas)
...
lambda <- function(t, covariates, N, betas) {
  if (length(covariates) == 0) {
    ...
    sp = exp(betas[1] + betas[length(betas)] * N)
  } else {
    ...
    sp = exp(betas[1] + sum(betas[2:(length(betas)-1)] * covariate_values) + betas[length(betas)] * N)
  }
  return(sp)
}
...
calc_D_stats = function(n_stats, n_pars, pars_i, n_trees_D,
                        covs, max_attempts){
  ...
  for (i in 1:n_trees_D){
    result = create_tree_mat_fixed_N(N_max, pars_i[1], pars_i[2:n_pars], covs, 0, max_attempts)
    tree_mat = result$tree_mat
  ...
  for (p in 1:n_pars){
    ...
    for (i in 1:n_trees_D){
      result = create_tree_mat_fixed_N(N_max, pars_eps[1], pars_eps[2:n_pars], covs, 0, max_attempts)
      tree_mat = result$tree_mat
      tree_extant = L2phylo(unname(tree_mat), dropextinct=TRUE)
...
# function that returns vectorized PDM statistics
stats_PDM = function(tree){
  PDM = calc_PDM(tree)
  PDM_ordered = reorder_PDM(PDM)
  PDM_stats = vectorize_PDM(PDM_ordered)
  return(PDM_stats)
}
